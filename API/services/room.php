<?php
    ...
    function createRoom($json) {
        ...
        return $sql->execute([$json['id'], $json['name'], $json['seats_number'], $json['type'], $json['floor_number'], $json['center_cif']]);
    }

    function editRoom($json) {
        $db = getDatabase();
        $sql = $db->prepare("UPDATE ROOM SET id = ?, name = ?, seats_number = ?, type = ?, floor_number = ?, center_cif = ? WHERE id = ?");
        return $sql->execute([$json['id'], $json['name'], $json['seats_number'], $json['type'], $json['floor_number'], $json['center_cif'], $json['id']]);
    }
